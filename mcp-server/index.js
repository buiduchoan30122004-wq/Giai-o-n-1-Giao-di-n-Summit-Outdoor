    ...
    if (name === "confirm_order") {
      ...
      if (resend && firstRow.customer_email) {
        try {
          ...
          console.error("Lỗi gửi email bằng Resend trong MCP:", err);
        }
      }
...
app.get("/sse", async (req, res) => {
  ...
  console.log("MCP Server connected to SSE client");
});
...
app.get("/health", (req, res) => {
  res.json({ status: "healthy", server: "summit-mcp-server" });
});

const PORT = process.env.PORT || 8000;
app.listen(PORT, () => {
  console.log(`Summit Outdoor MCP Server running on port ${PORT}`);
